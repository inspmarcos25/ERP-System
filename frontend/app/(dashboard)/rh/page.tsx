    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 text-center rounded-2xl">
          ...
          <h3 className="text-sm font-semibold text-slate-800 dark:text-white">Nenhum funcionário cadastrado</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Clique em admitir colaborador para começar o onboarding.</p>
        </div>
      ) : viewMode === 'list' ? (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-900/40">
                  ...
                  <th className="text-left py-3 px-4 text-slate-500 dark:text-slate-400 font-semibold tracking-wider uppercase">Departamento</th>
                  <th className="text-left py-3 px-4 text-slate-500 dark:text-slate-400 font-semibold tracking-wider uppercase">Cargo</th>
                  <th className="text-right py-3 px-4 text-slate-500 dark:text-slate-400 font-semibold tracking-wider uppercase">Salário</th>
                  <th className="text-left py-3 px-4 text-slate-500 dark:text-slate-400 font-semibold tracking-wider uppercase">Admissão</th>
              ...
              <tbody>
                {funcionarios.map((f) => {
                  ...
                  return (
                    <tr key={f.id} className="border-b border-slate-100 dark:border-slate-800/30 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-800 dark:text-white font-semibold">{f.nome}</span>
                        </div>
                      </td>
                      <td className="py-3 px-4 text-slate-500 dark:text-slate-400">{f.email || '-'}</td>
                      <td className="py-3 px-4 text-slate-500 dark:text-slate-400">{f.departamento || '-'}</td>
                      ...
                      <td className="py-3 px-4 text-slate-500 dark:text-slate-400">{f.data_admissao ? new Date(f.data_admissao).toLocaleDateString('pt-BR') : '-'}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {funcionarios.map((f) => {
            const cargo = cargos.find(c => c.id === f.cargo_id);
            return (
              <div key={f.id} className="glass-panel p-5 rounded-2xl flex flex-col justify-between border-slate-200 dark:border-slate-800/80">
                <div>
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-800 dark:text-white text-sm line-clamp-1">{f.nome}</h3>
                  ...
                  <div className="space-y-1 text-xs text-slate-500 dark:text-slate-400 mt-4">
                    {f.email && (
                      <div className="flex items-center gap-2">
      ...
      {showAddFunc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 dark:bg-slate-950/85 backdrop-blur-sm">
          ...
          >
            <h3 className="text-md font-bold text-slate-800 dark:text-white mb-2">Admitir Colaborador</h3>
            
            <div className="space-y-3">
              <div>
                <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Nome Completo</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">CPF</label>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">E-mail Corporativo</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Cargo</label>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Departamento</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Salário Contratual (R$)</label>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Data Admissão</label>
            ...
            <div className="flex justify-end gap-2 pt-3">
              <button type="button" onClick={()=>setShowAddFunc(false)} className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-850 text-slate-500 dark:text-slate-400 text-xs cursor-pointer">Cancelar</button>
      ...
      {showEditFunc && editFuncData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 dark:bg-slate-950/85 backdrop-blur-sm">
          <form 
            ...
            className="w-full max-w-md glass-panel bg-white dark:bg-[#0B0F19] p-6 rounded-2xl shadow-2xl space-y-4"
          >
            <h3 className="text-md font-bold text-slate-800 dark:text-white mb-2">Editar Colaborador</h3>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Nome Completo</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Salário (R$)</label>
            ...
            <div className="flex justify-end gap-2 pt-3">
              <button type="button" onClick={()=>{setShowEditFunc(false); setEditFuncData(null)}} className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-850 text-slate-500 dark:text-slate-400 text-xs cursor-pointer">Cancelar</button>
