...
export default function FinanceiroPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Control Actions Bar */}
      <div className="flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/20 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/40">
        <h4 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
          Listagem de Lançamentos ({activeTab === 'receber' ? 'Receber' : 'Pagar'})
        </h4>
        <button
          onClick={() => setShowAddConta(true)}
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-slate-800 dark:text-white text-xs font-semibold shadow-lg shadow-indigo-600/25 cursor-pointer transition-colors"
        >
          <Plus size={15} />
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 text-center rounded-2xl">
          ...
          <h3 className="text-sm font-semibold text-slate-800 dark:text-white">Nenhum lançamento pendente</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Clique em registrar para adicionar uma movimentação.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
                {contas.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                    <td className="px-5 py-4 text-slate-800 dark:text-white font-semibold">
                      <p>{c.descricao}</p>
                    ...
                    <td className="px-5 py-4 text-slate-800 dark:text-white font-bold font-mono">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 dark:bg-slate-950/85 backdrop-blur-sm">
          ...
          >
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Descrição / Título</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Valor (R$)</label>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Vencimento</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Nº Documento</label>
            ...
            <div className="flex justify-end gap-2 pt-3">
              <button type="button" onClick={()=>setShowAddConta(false)} className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-850 text-slate-500 dark:text-slate-400 text-xs cursor-pointer">Cancelar</button>
              <button type="submit" className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-slate-800 dark:text-white text-xs cursor-pointer">Registrar</button>
            </div>
          </form>
        </div>
      )}
      ...
      {showEditConta && editContaData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 dark:bg-slate-950/85 backdrop-blur-sm">
          <form 
            ...
            className="w-full max-w-md glass-panel bg-white dark:bg-[#0B0F19] p-6 rounded-2xl shadow-2xl space-y-4"
          >
            <h3 className="text-md font-bold text-slate-800 dark:text-white mb-2">Editar Lançamento</h3>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">Descrição / Título</label>
            ...
            <div className="flex justify-end gap-2 pt-3">
              <button type="button" onClick={()=>{setShowEditConta(false); setEditContaData(null)}} className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-850 text-slate-500 dark:text-slate-400 text-xs cursor-pointer">Cancelar</button>
